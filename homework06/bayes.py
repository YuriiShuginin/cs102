...
class NaiveBayesClassifier:
    ...
    def fit(self, X, y):
        ...
        for _ in words:
            for i in range(len(self.classes)):
                self.defdict[_].append((self.defdict[_][i] + self.a) / (class_word_am[self.classes[i]] + len(self.defdict) * self.a))
            ...
            for cl in self.classes:
                ...
                for word in msg.split():
                    if word in self.defdict:
                        prob += math.log(self.defdict[word][self.classes.index(cl) + len(self.classes)])
                probs_cl[cl] = prob
